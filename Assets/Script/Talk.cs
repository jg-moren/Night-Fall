...
public class Talk : MonoBehaviour
{
    ...
    int idfrase;//controla em qual frase o player esta
    ...
    {
        if (collision.gameObject.tag == "Player") //ao colidir com um gameobject com a tag "Player" starta a fala
        {
    ...
    private void Update()
    {
        ...
        if (0 <= idfrase && idfrase < dialogues.sentence.Length)
        {
            if (dialogues.sentence[idfrase].hasAnswer)
            {
                ...
                if (Input.GetKeyDown(KeyCode.S) && idAnswer > -4)idAnswer -= 1;
                setAnswer();
            }
            else
            {
    ...
    void setDialog()
    {
        ...
        if (0 <= idfrase && idfrase < dialogues.sentence.Length)
        {
            ...
            if (dialogues.sentence[idfrase].hasAnswer)//se tiver resposta
            {
                ...
                StartCoroutine(write(dialogues.sentence[idfrase].answers[0].frase, answer0));
                StartCoroutine(write(dialogues.sentence[idfrase].answers[1].frase, answer1));
                StartCoroutine(write(dialogues.sentence[idfrase].answers[2].frase, answer2));
                StartCoroutine(write(dialogues.sentence[idfrase].answers[3].frase, answer3));
